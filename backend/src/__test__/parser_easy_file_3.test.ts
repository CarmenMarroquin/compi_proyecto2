...
describe('Test Interpreter On Easy 3', () => {
    let globalEnv: Environment;
    let tree: Tree;

    beforeAll(() => {
        // Reading test file
        const testPath = path.join(__dirname, '..', '..', 'testFiles', 'facil_3.test.ci');
   ...
   afterAll(() => {
        ...
        const htmlFilePath = path.join(__dirname, 'easy_file_3_output.html');
